...
class pdfit
{
	...
	function GetOrderItems()
	{
		...
		foreach ($this->order->getBasket()->getBasketItems() as $basketItem)
		{
			...
			$this->pdf->Cell($coloumns[2]['w'], 6, CCurrencyLang::CurrencyFormat($basketItem->getPrice(), 'RUB', true), 
				'LR', 0, 'L', $fill);
			$this->pdf->Cell($coloumns[3]['w'], 6, number_format($basketItem->getQuantity()), 'LR', 0, 'C', $fill);
			$this->pdf->Cell($coloumns[4]['w'], 6, CCurrencyLang::CurrencyFormat($basketItem->getFinalPrice(), 'RUB', 
				true), 'LR', 0, 'L', $fill);
			$this->pdf->Ln();
			$fill = !$fill;
		}
		...
		$this->pdf->Ln();
		$this->pdf->SetFont('dejavusans', 'U',14);
